...
module.exports = {
  ...
  testBasicAuth(test) {
    server = http.createServer((req, res) => {
      ...
      axios.get(`http://${user}@localhost:4444/`, { headers }).then((res) => {
        const base64 = new Buffer.From(`${user}:utf8`).toString('base64');
  ...
  testBasicAuthWithHeader(test) {
    server = http.createServer((req, res) => {
      ...
      axios.get('http://localhost:4444/', { auth, headers }).then((res) => {
        const base64 = new Buffer.From('foo:bar', 'utf8').toString('base64');
        test.equal(res.data, `Basic ${base64}`);
        test.done();
      });
    });
  ...
  testBuffer(test) {
    const buf = new Buffer.From(1024); // Unsafe buffer < Buffer.poolSize (8192 bytes)
    buf.fill('x');
    server = http.createServer((req, res) => {
